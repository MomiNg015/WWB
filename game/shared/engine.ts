...
import { canDefend, defenseError, defenseSpecial } from "./defense";
export { canDefend } from "./defense";
import { classicRules, itemById, items } from "./catalog";
import type {
  Action,
  Card,
  Command,
...
function hurt(
  ...
  if (actual > 0) {
    ...
    if (p.guardian && random(g) < g.rules.guardianLeaveChance) {
      g.cues.push({ kind: "guardianLeave", target: index, label: p.guardian });
      p.guardian = null;
    }
  }
...
function resolveDeath(g: Game, index: number) {
  ...
  if (p.hp === 0) {
    ...
    if (rev) {
      ...
      g.cues.push({ kind: "heal", target: index, amount: p.hp, hpAfter: p.hp });
      log(g, `${p.name} 复活`);
    } else {
      log(g, `${p.name} 升天`);
...
export function combineElements(elements: Element[]): Element {
...
export const effects: Record<string, EffectHandler> = {
  ...
  setGuardian: (g, s, t) => {
    ...
    g.cues.push({ kind: "guardian", target: t, label: g.players[t].guardian! });
  },
  removeItems: (g, s, t) => {
    const p = g.players[t];
  ...
  callPhenomenon: (g, s) => {
    ...
    if (item.atk)
      queueAttack(
        g,
        s,
        item.ability === "attackSomebody" ? [pick(g, alive(g))] : enemies(g, s),
        [item],
      );
...
function guardianAction(g: Game, index: number, original: Item) {
  ...
  if (p.guardian === "moon") {
    ...
    if (d.ability === "exchange") {
      ...
      p.hp = lowHP + Math.floor(random(g) * (Math.min(total, max) - lowHP + 1));
...
function play(g: Game, index: number, cmd: Extract<Command, { type: "play" }>) {
  const p = g.players[index];
...
function defend(
  ...
  if (a.kind === "sell") {
    ...
    for (const d of a.cards) {
      if (damage > 0 && d.ability === "addCurseOnDamage") curse(g, p, d.curse!);
      if (
        d.ability === "absorbHP" &&
...
export function reduceGame(
  ...
  if (g.phase === "defense") {
    if (g.queue[0].kind === "purchase") {
      if (command.type !== "purchase") throw new Error("请选择购买或放弃购买");
    ...
    if (command.type === "play") play(g, index, command);
      ...
      if (
        g.players[index].hand.some((c) => itemById(c.id).category === "weapons")
      )
  ...
  g.players.forEach((p, i) => {
    for (const stat of ["mp", "cp"] as const) {
      ...
      const already =
        ...
        g.cues
          .filter(
            (c) => c.kind === "resource" && c.target === i && c.label === label,
          )
...
export function projectGame(g: Game, playerId: string): GameView {
  ...
  return {
    ...
    players: g.players.map((p, i) => {
      ...
      return {
        ...
        ...(i === self
          ? {
              hand: hand.map((c) => ({ uid: c.uid, id: c.apparentId || c.id })),
...
export function botCommand(g: Game, index: number): Command {
  const p = g.players[index];
